  ...
  return (
    <div className="space-y-10">
      <section className="grid gap-4 border border-white/10 p-4 md:grid-cols-[1fr_220px_220px]">
        ...
        <FilterSelect
          label={props.category === "vowel" ? "Backness" : "Place"}
          value={primaryFilter}
        ...
        <FilterSelect
          label={props.category === "vowel" ? "Height" : "Manner"}
      ...
      <div className="flex items-center justify-between border-b border-white/10 pb-4">
        <p className="text-sm text-white/50">
          ...
          of <span className="font-medium text-white">{props.symbols.length}</span>
        </p>
        <button
          type="button"
          onClick={() => {
      ...
      {filteredSymbols.length > 0 ? (
        props.category === "vowel" ? (
          <VowelGroups symbols={filteredSymbols as VowelSymbol[]} />
        ) : (
          <ConsonantGroups symbols={filteredSymbols as ConsonantSymbol[]} />
        )
      ) : (
...
function VowelGroups({ symbols }: { symbols: VowelSymbol[] }) {
...
function ConsonantGroups({ symbols }: { symbols: ConsonantSymbol[] }) {
